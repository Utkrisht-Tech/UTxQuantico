...
import MathX
import MathX.complex as cmplx

fn testXcomplex_addition() {
...
fn testXcomplex_multiplication() {
	// Test is based on and verified from practice examples of Khan Academy 
	// https://www.khanacademy.org/math/precalculus/imaginary-and-complex-numbers
...
fn testXcomplex_abs() {
	...
	assert c1.abs().eq(MathX.sqrt(5))
	assert c1.abs().eq(c1.conjugate().abs())
	c1 = cmplx.complex(7,0)
	assert c1.abs() == 7
}

fn testXcomplex_angle(){
	// Test is based on and verified from practice examples of Khan Academy 
	// https://www.khanacademy.org/math/precalculus/imaginary-and-complex-numbers
	mut c := cmplx.complex(1, 0)
	assert (c.angle() * 180 / MathX.pi).eq(0)
	c = cmplx.complex(1, 1)
	assert (c.angle() * 180 / MathX.pi).eq(45)
	c = cmplx.complex(0, 1)
	assert (c.angle() * 180 / MathX.pi).eq(90)
	c = cmplx.complex(-1, 1)
	assert (c.angle() * 180 / MathX.pi).eq(135)
	c = cmplx.complex(-1, -1)
	assert (c.angle() * 180 / MathX.pi).eq(-135)
	mut cc := c.conjugate()
