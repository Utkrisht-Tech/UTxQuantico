// TODO: Remove this Go style urllib and implement UTxQ\'s
// own more advance and powerful urllib named URLxLib.
...
// digits.
public fn query_unescape(s string) ?string {
...
// unescape '+' to ' ' (space).
public fn path_unescape(s string) ?string {
...
fn unescape(s_ string, mode EncodingMode) ?string {
	...
	mut t := StringX.new_builder(s.len - 2*n)
	for i := 0; i < s.len; i++ {
		x := s[i]
		switch x {
		case `%`:
...
// inside a URL query.
public fn query_escape(s string) string {
...
public fn path_escape(s string) string {
...
// and no password set.
public fn user(username string) &Userinfo {
	return &Userinfo{
		username: username,
		password: '',
		password_set: false
...
public fn parse(rawurl string) ?URL {
	// Cut off #frag
	p := split(rawurl, '#', true)
	u := p[0]
	frag := p[1]
...
fn _parse(rawurl string, via_request bool) ?URL {
	...
	// if rest.ends_with('?') && StringX.count(rest, '?') == 1 {
...
public fn (u &URL) str() string {
	mut buf := StringX.new_builder(200)
	if u.scheme != '' {
		buf.write(u.scheme)
		buf.write(':')
	}
...
public fn (v Values) encode() string {
	if v.size == 0 {
		return ''
	}
	mut buf := StringX.new_builder(200)
...
public fn (u &URL) parse(ref string) ?URL {
	refurl := parse(ref) or {
		return error(err)
	}
...
public fn (u &URL) resolve_reference(ref &URL) ?URL {
	mut url := *ref
	if ref.scheme == '' {
		url.scheme = u.scheme
	}
...
public fn (u &URL) request_uri() string {
	mut result := u.opaque
	if result == '' {
...
public fn (u &URL) hostname() string {
...
public fn valid_userinfo(s string) bool {
	for r in s {
		if `A` <= r && r <= `Z` {
			continue
		}
