...
// Create socket
public fn socket(family int, _type int, proto int) ?Socket {
	$if windows {
...
public fn socket_udp() ?Socket {
	return socket(C.AF_INET, C.SOCK_DGRAM, C.IPPROTO_UDP)
}

// Set socket options
public fn (s Socket) setsockopt(level int, optname int, optvalue &int) ?int {
...
// Bind socket to port
public fn (s Socket) bind(port int) ?int {
...
public fn (s Socket) listen_backlog(backlog int) ?int {
	mut n := 0
	if backlog > 0 {
		n = backlog
	}
...
public fn listen(port int) ?Socket {
	$if debug {
		println('NetX.listen($port)')
	}
...
public fn (s Socket) accept() ?Socket {
	$if debug {
		println('accept()')
	}
...
public fn (s Socket) connect(address string, port int) ?int {
...
public fn dial(address string, port int) ?Socket {
	s := socket(C.AF_INET, C.SOCK_STREAM, 0) or {
		return error(err)
	}
...
public fn (s Socket) cread( buffer byteptr, buffersize int ) int {
...
public fn (s Socket) crecv( buffer byteptr, buffersize int ) int {
	return int( C.recv(s.sockfd, buffer, buffersize, 0) )
}

// Shutdown and close socket
public fn (s Socket) close() ?int {
	mut shutdown_res := 0
	$if windows {
