...
public fn(dset mut DepSet) add(item string) {
	dset.items << item
}

public fn(dset &DepSet) diff(otherset DepSet) DepSet {
	mut diff := DepSet{}
...
public fn(dset &DepSet) size() int {
	return dset.items.len
}

public fn new_dep_graph() &DepGraph {
	return &DepGraph{
		acyclic: true
	}
}
...
public fn(graph &DepGraph) resolve() &DepGraph {
...
public fn(graph &DepGraph) last_node() DepGraphNode {
	return graph.nodes[graph.nodes.len-1]
}

public fn(graph &DepGraph) last_cycle() string {
	return graph.last_node().last_cycle
}

public fn(graph &DepGraph) display() {
