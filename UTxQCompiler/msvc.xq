...
fn find_vs() ?VsInstallation {
	...
	if os.file_exists('$lib_path\\vcruntime.lib') {
		path := '$res.output\\VC\\Tools\\MSVC\\$v\\bin\\Hostx64\\x64'
	...
	println('Unable to find Visual Studio installation (attempted to use lib path "$lib_path")')
...
public fn (xQ mut UTxQ) cc_msvc() {
	...
	out_name_obj := os.realpath( xQ.out_name_c + '.obj' )
	...
	if xQ.pref.is_so {
		if !xQ.out_name.ends_with('.dll') {
			xQ.out_name = xQ.out_name + '.dll'
		}

		// Build dll
		a << '/LD'
	} else if !xQ.out_name.ends_with('.exe') {
		xQ.out_name = xQ.out_name + '.exe'
	}
	...
	if xQ.pref.build_mode == .build {
		...
		b := os.realpath( '$ModPath/xQLib/builtin.obj' )
		alibs << '"$b"'
		if !os.file_exists(b) {
		...
		for imp in xQ.table.imports {
			...
			alibs << '"' + os.realpath( '$ModPath/xQLib/${imp}.obj' ) + '"'
		}
	}
	...
	//a << '"$TmpPath/$xQ.out_name_c"'
	a << '"' + os.realpath( xQ.out_name_c ) + '"'
