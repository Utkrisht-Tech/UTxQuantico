...
pub fn(graph mut ModDepGraph) from_import_tables(import_tables []ParsedImportsTable) {
	for pit in import_tables {
		mut deps := []string
...
// 'strings' => 'UTxQuantico/UTxQLib/strings'
...
fn (xQ &UTxQ) find_module_path(mod string) string {
	...
	if !os.dir_exists(import_path) {
		import_path = '$xQ.lang_dir/UTxQLib/$mod_path'
	}
